...
void arduino_coordinates_callback(const geometry_msgs::Point &msg) {
  ...
  double temp_position_x = msg.x;
  float temp_position_y = msg.y;
  //dtostrf();
  float temp_position_z = msg.z;
...
ros::Publisher gewas_bakken_vol_pub("/storage_is_full", &empty_msg);         
ros::Publisher point_pub("/point_topic", &point_msg);
